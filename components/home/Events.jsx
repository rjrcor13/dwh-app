...
const Events = () => {
	...
	const sortedEvents = [...hospitalCelebrations].sort((a, b) => new Date(b.date) - new Date(a.date));

	const loadMore = () => {
		setVisibleEvents((prev) => prev + 3);
	};

	// Helper to get day and month for badge
	...
	const jsonLd = {
		...
		"@type": "ItemList",
		"itemListElement": sortedEvents.slice(0, visibleEvents).map((event, index) => ({
			"@type": "ListItem",
			"position": index + 1,
			"item": {
				"@type": "Event",
				...
				"endDate": event.date, // Assuming single day for now
				...
				"url": `https://divinewordhospital.com/events/hospital-celebrations/${event.id}`
			}
		}))
	};
		...
		<section className="py-24 bg-white relative overflow-hidden">
			...
			<div className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 ">
				...
				<div className="grid gap-10 md:grid-cols-2 lg:grid-cols-3">
					{sortedEvents.slice(0, visibleEvents).map((event, idx) => {
						const { day, month } = getDateParts(event.date);
						const eventLink = `/events/hospital-celebrations/${event.id}`;
						...
						return (
							...
							>
								<div className="relative h-64 overflow-hidden">
									<Image
										src={event.media[0]?.src || '/assets/events.jpg'} // Fallback image
										alt={event.title}
										fill
									...
									<div className="absolute top-4 left-4 bg-white/95 backdrop-blur text-slate-900 rounded-2xl p-3 text-center min-w-[70px] shadow-lg border border-slate-100">
										<span className="block text-xs font-bold uppercase tracking-wider text-slate-500">{month}</span>
										<span className="block text-2xl font-bold font-heading text-primary leading-none">{day}</span>
									</div>
