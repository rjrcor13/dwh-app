...
const DoctorProfile = ({ slug: propSlug }) => {
	...
	return (
		<div className="min-h-screen bg-slate-50/50 relative font-sans selection:bg-blue-100 pb-20">
			...
			{/* 1. HERO SECTION (New Premium Header) */}
			<div className="relative z-10 max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-12 md:pt-20 mb-12 text-center">
				<motion.div
				...
				>
					<div className="flex justify-center mb-6">
						<PremiumBadge text="Medical Specialist" icon={Stethoscope} />
					</div>

					<h1 className="text-4xl md:text-5xl lg:text-7xl font-bold font-heading text-slate-900 mb-6 tracking-tight">
						{doctor.name}
					</h1>
					...
					<div className="flex flex-wrap justify-center gap-3 mb-8 max-w-2xl mx-auto">
						{Array.isArray(doctor.specialties) ? (
							doctor.specialties.map((spec, index) => (
								<Badge key={index} variant="secondary" className="bg-white/80 backdrop-blur border border-blue-100 text-primary px-4 py-2 text-base font-bold shadow-sm">
									{spec}
								</Badge>
							))
						) : (
							<Badge variant="secondary" className="bg-white/80 backdrop-blur border border-blue-100 text-primary px-4 py-2 text-base font-bold shadow-sm">
								{doctor.specialties}
							</Badge>
						)}
					</div>
			...
			<div className="relative z-10 max-w-[1440px] mx-auto px-6">
				<div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
					...
					>
						<div className="bg-white/60 backdrop-blur-xl border border-white/60 rounded-[2.5rem] p-8 shadow-xl shadow-slate-200/50 sticky top-[200px] text-center">
							{/* Large Avatar */}
							<div className="relative inline-block mb-8 group w-full max-w-[280px]">
								<div className="absolute inset-0 bg-blue-500/10 rounded-[2rem] blur-xl group-hover:bg-blue-500/20 transition-all duration-500" />
								<Avatar className="w-full h-auto aspect-square rounded-[2rem] border-4 border-white shadow-2xl mx-auto relative z-10">
									<AvatarImage
										src={doctor.image}
										alt={doctor.name}
										className="object-cover"
									/>
									<AvatarFallback className="bg-blue-50 text-blue-600 text-5xl font-heading rounded-[2rem]">
										{doctor.name ? doctor.name.substring(0, 2).toUpperCase() : 'DR'}
									</AvatarFallback>
								</Avatar>
								<div className="absolute bottom-4 right-4 w-6 h-6 bg-emerald-500 border-4 border-white rounded-full z-20 shadow-sm" title="Available Today" />
							</div>
					...
					>
						...
						<div className="bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-xl shadow-slate-100/50 relative overflow-hidden">
							<div className="absolute top-0 right-0 w-80 h-80 bg-blue-50/50 rounded-full blur-[90px] -translate-y-1/2 translate-x-1/2 opacity-50" />
						...
						<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
							...
							{/* Clinic Schedule */}
							<div className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-100 shadow-xl shadow-slate-100/50 flex flex-col h-full relative overflow-hidden">
								<h3 className="text-lg font-bold text-slate-900 mb-8 flex items-center gap-3 relative z-10">
							...
							<div className="bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-100 shadow-xl shadow-slate-100/50 flex flex-col h-full relative overflow-hidden">
								...
								<div className="flex flex-wrap content-start gap-3 h-full relative z-10">
